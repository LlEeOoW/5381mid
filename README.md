...
**Live demo (DigitalOcean):** [https://midterm5381-ecv6k.ondigitalocean.app/](https://midterm5381-ecv6k.ondigitalocean.app/) — see [DEPLOYMENT.md](DEPLOYMENT.md) for details.
...
- **DigitalOcean:** Run the app as a Web Process (e.g. `uvicorn main:app --host 0.0.0.0 --port $PORT`). Add Supabase and OpenAI env vars in the DO app dashboard. Use a static buildpack or Dockerfile that installs `requirements.txt` and runs the above command.
  - **Important:** The **entire repo** (including the `templates/` folder and `templates/dashboard.html`) must be deployed. In App Platform, do **not** set a "Source Directory" that points inside a subfolder that excludes `templates/`. Deploy from the repo root so that `main.py` and `templates/dashboard.html` are in the same deployment (e.g. `/workspace/main.py` and `/workspace/templates/dashboard.html`). If you see "Dashboard file not found" at `/`, the server is missing the `templates` folder—push it to GitHub and redeploy.
